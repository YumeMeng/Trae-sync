# Gate B Run - T02 account evidence and UI boundary (T02 Repair 4: evidence closure)
...
#
# Repair 4 (evidence closure) vs Repair 3 (20260802-193000):
#   - 不修改生产代码或测试
...
$gateDir = "$repoRoot\evidence\gates\B\20260802-220000"
...
try {
    ...
    # Step 17: working-tree binding — T02 源码逐文件 SHA-256 + 纯 git diff --binary 哈希
    $wtHashes = @()
    ...
    Write-Host "Step 17: diffPath = $diffPath"
    $outputArg = "--output=" + $diffPath
    Write-Host "Step 17: outputArg = $outputArg"
    try {
        & git -C $repoRoot diff --binary $outputArg
        $gitExit = $LASTEXITCODE
        Write-Host "Step 17: git diff exit = $gitExit"
    ...
    "git diff --binary SHA-256 (pure, via --output tempfile): $gitDiffHash" | Add-Content -Path "$logsDir\17-working-tree-binding.log" -Encoding UTF8
...
$envJson = @{
    ...
    scope = "Account evidence matrix + UI boundary (T02 Repair 4: evidence closure, pure git diff hash + closure marker)"
    working_tree = @{
...
$assertions = @{
    ...
    acceptance_criteria = @{
        ...
        "R2-4_working_tree_binding" = @{ status = if ($phase1Success) { "PASS" } else { "FAIL" }; evidence = "working-tree-hashes.sha256 ($($t02SourceFiles.Count) files) + pure git diff --binary SHA-256 (via --output tempfile) recorded; combined=$workingTreeHash gitdiff=$gitDiffHash" }
