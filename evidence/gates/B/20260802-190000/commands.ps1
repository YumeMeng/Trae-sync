...
#
# 20260802-190000 vs 20260802-181000 (修复 PowerShell strict mode bug):
#   - 20260802-181000 因 Set-StrictMode -Version Latest + 外部命令 stderr ErrorRecord
#     累积导致 Step 13 变量赋值失败（$realPathScanFiles 未设置），证据不完整
#   - 本运行在 Step 13 之前关闭 strict mode (Set-StrictMode -Off)，修复此问题
#   - 保留 20260802-181000 失败证据作为历史记录，不删除、不修改
#
...
$gateDir = "$repoRoot\evidence\gates\B\20260802-190000"
...
try {
    ...
    Write-Host "Run ID: 20260802-190000"
    ...
    # 修复 PowerShell strict mode bug：外部命令 (cargo/pnpm) 产生的 stderr ErrorRecord
    # 在 Set-StrictMode -Version Latest + $ErrorActionPreference=Stop 下累积，
    # 导致后续变量赋值失败。在安全扫描步骤前关闭 strict mode。
...
# ===================== generate REPORT.md =====================
$report = @"
# Gate B Run Report - 20260802-190000
