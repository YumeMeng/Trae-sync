...
- **方案**：页面头部刷新按钮 = 重读账号总览+登录凭据状态（`refreshOverview`），不发网络请求，按钮带旋转动画。语义分工：刷新=重读缓存；健康检测=真实探测。签到页同构添加。不做自动轮询。
...
**G3 设置页折叠收尾**（若 P8-1 未覆盖）：密钥区块折叠后只露一行结论（"密钥正常"/"密钥异常需维护"）；备份绝对路径收 `<details>`；正文一句话+立即备份按钮。

**G4 残余文案**：插件卡片标签改名（随主库/仅此账号→切号保留/仅此账号）随 G23 一并落地（对账条文案已被 G23 取代作废）。
